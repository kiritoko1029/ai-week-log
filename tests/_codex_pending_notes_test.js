'use strict'
/* Codex hook pending-note pool tests: storage, write, and AI summarization. */
const fs = require('fs')
const os = require('os')
...
function tmpDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'wl-codex-pending-'))
}
  ...
  console.log('\n[1] pending note storage')
  {
    ...
    try {
      const saved = P.addPendingNote(dir, {
        ...
        summary: '完成 Codex hook 待处理小记池。',
        branch: 'codex/hook-notes',
        changedFiles: ['src/main/ipc.js', 12, ''],
        finishedAt: '2026-06-17T08:00:00.000Z',
      }, cfg())
      ok('保存候选项返回 id', /^cpn_/.test(saved.id), saved.id)
      ok('根据 cwd 匹配项目名', saved.project === 'WeekLog', saved.project)
      ok('使用 finishedAt 作为 createdAt', saved.createdAt === '2026-06-17T08:00:00.000Z', saved.createdAt)
  ...
  console.log('\n[3] summarize selected notes')
  {
    ...
    try {
      const a = P.addPendingNote(dir, {
        ...
        changedFiles: ['src/main/codex-hook-server.js'],
      }, cfg())
      const b = P.addPendingNote(dir, {
        cwd: '/Users/me/apps/weeklog',
      ...
      const provider = {
        summarize: async (system, user) => {
          seenSystem = system
          seenUser = user
          return { text: '完成 Codex hook 小记待处理池，包含安全接入和批量处理界面。', model: 'fake' }
        },
      }
  ...
  console.log('\n[4] write AI summary once for selected notes')
  {
    ...
    try {
      const a = P.addPendingNote(dir, {
        ...
        finishedAt: '2026-06-17T09:00:00.000Z',
      }, cfg())
      const b = P.addPendingNote(dir, {
        cwd: '/Users/me/apps/weeklog',
        summary: '完成待处理池界面。',
      ...
      const result = P.writePendingNotes(dir, {
        ...
        miscProject: '日常工作',
        project: 'WeekLog',
        content: '完成 Codex hook 小记待处理池，支持安全接入、批量确认和 AI 汇总录入。',
      })
      const text = N.getNoteText(notesDir, '2026-06-17')
      const occurrences = (text.match(/完成 Codex hook 小记待处理池/g) || []).length

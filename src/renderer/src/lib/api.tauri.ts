...
export const api: WeeklogAPI = {
  ...
  memory: {
    ...
    downloadModel: () => invoke<{ ok: boolean; error?: string; model?: string; cacheDir?: string; modelDir?: string; sizeMB?: number }>('memory_download_model'),
    openModelFolder: () => invoke<{ ok: boolean; path?: string }>('memory_open_model_folder'),
    clearModel: () => invoke<{ ok: boolean; model?: string; path?: string }>('memory_clear_model'),

...
const { sanitizeZcodeSummary } = require('./zcode-summary')

const FILE = 'zcode-notes-pending.json'
...
function newId() {
  return 'zpn_' + Date.now().toString(36) + Math.random().toString(36).slice(2, 8)
}
...
function readStore(dir) {
  try {
    ...
    console.error('[weeklog] ZCode 待处理小记读取失败：', e.message)
  }
...
function normalizePayload(payload = {}, cfg = {}) {
  const summary = sanitizeZcodeSummary(payload.summary || payload.content || payload.text)
...
function normalizeStoredItem(item) {
  ...
  return {
    ...item,
    summary: sanitizeZcodeSummary(item.summary),
  }
}
...
function contentForItem(item) {
  const files = Array.isArray(item.changedFiles) && item.changedFiles.length
    ? `（ZCode 自动小记；分支：${item.branch || '未知'}；改动文件：${item.changedFiles.slice(0, 6).join('、')}${item.changedFiles.length > 6 ? ` 等 ${item.changedFiles.length} 个` : ''}）`
    : ''

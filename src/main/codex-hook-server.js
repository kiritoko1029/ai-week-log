...
// @ts-check
/**
 * 本地 Codex hook HTTP 入口。
 *
 * 仅绑定 127.0.0.1；外部 hook 必须携带本机 token，写入待处理池而非正式笔记。
 */
const http = require('http')
const pending = require('./codex-pending-notes')
...
function createCodexHookServer({ dir, getConfig, getToken, logger } = {}) {
  let server = null
  ...
  const handle = async (req, res) => {
    ...
    const hookCfg = cfg.codexHook || {}
    if (!hookCfg.enabled) {
      json(res, 403, { error: 'Codex hook 未启用' })
      return
    }
    if (req.method !== 'POST' || req.url !== '/api/codex/pending-notes') {
    ...
    try {
      ...
      if (logger) logger.info('codex.hook', '收到 Codex 待处理小记', { id: item.id, project: item.project, cwd: item.cwd })
      json(res, 201, { id: item.id, item })
    } catch (e) {
      if (logger) logger.warn('codex.hook', 'Codex 待处理小记写入失败', { error: e.message })
  ...
  const applyConfig = async () => {
    ...
    await new Promise((resolve) => {
      server.once('error', (err) => {
        ...
        if (logger) logger.error('codex.hook', 'Codex hook 本地服务启动失败', { error: err.message, port: desiredPort })
        server = null
        resolve()
      })
      server.listen(desiredPort, HOST, () => {
        ...
        if (logger) logger.info('codex.hook', 'Codex hook 本地服务已启动', { port: status.port })
        resolve()
      })
    })
    return status
